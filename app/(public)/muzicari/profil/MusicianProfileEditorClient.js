...
import { ArrowLeft, Save, UserRound, Mail, CalendarDays, Euro, MapPin, Music, Video, Image as ImageIcon, ListMusic, Radio, Headphones, Trash2, QrCode, HelpCircle, Download, ChevronDown, ExternalLink } from 'lucide-react';
...
export default function MusicianProfileEditorClient({ mode = 'panel' }) {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      window.location.href = '/muzicari/profil?saved=1';
    } catch (err) {
            ...
            <form onSubmit={onSubmit} className="profile-editor" style={sCard}>
              ...
              {viewer?.plan && (viewer.plan.toUpperCase() === 'PREMIUM' || viewer.plan.toUpperCase() === 'PREMIUM_VENUE') && (
                <>
                  ...
                  <div className="field toggle-field">
                    <label className="toggle-label" htmlFor="musician-allow-full-repertoire-live">
                    ...
                    <button
                      type="button"
                      id="musician-allow-full-repertoire-live"
                      role="switch"
                      aria-checked={form.allowFullRepertoireLive}
                      className={`tips-toggle ${form.allowFullRepertoireLive ? 'on' : ''}`}
                      onClick={() => onChange('allowFullRepertoireLive', !form.allowFullRepertoireLive)}
                    >
                      <span className="tips-toggle-knob" />
                    </button>
                  </div>
              ...
              <div className="media-grid">
                <div className="field">
                  ...
                  {form.img ? (
                    <>
                      <img src={form.img} alt="Preview" className="preview-image" />
                      <button type="button" className="btn-remove-media" onClick={() => onChange('img', '')}>
                ...
                <div className="field">
                  ...
                  <div className="field-hint-row">
                    ...
                    {form.videoUrl && (
                      <button type="button" className="btn-remove-media" onClick={() => onChange('videoUrl', '')}>
            ...
            <section className="musician-panel">
              <div className="panel-hero">
                ...
                {isPremiumVenue && korgPaItems.length ? (
                  <div className="korg-accordion" style={{ marginTop: '1rem' }}>
                    <button
                      type="button"
                      className="korg-accordion-toggle"
                      onClick={() => setShowKorgDownloads((prev) => !prev)}
                    >
                      <div className="panel-icon" style={{ width: 36, height: 36 }}>
                        <Download size={18} />
                      </div>
                      <div style={{ flex: 1, textAlign: 'left' }}>
                        <h3 style={{ margin: 0, fontSize: '0.92rem', fontWeight: 800, color: '#f8fafc' }}>Korg PA setovi</h3>
                        <p style={{ margin: 0, fontSize: '0.78rem', color: 'rgba(226,232,240,0.6)' }}>
