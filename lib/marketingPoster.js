...
async function loadFontBase64(weight) {
  if (weight === 'bold' && fontCacheBold) return fontCacheBold;
  if (weight !== 'bold' && fontCacheRegular) return fontCacheRegular;

  // Try local file first
  const localPath = join(process.cwd(), 'public', 'fonts', weight === 'bold' ? 'Inter-Bold.ttf' : 'Inter-Regular.ttf');
  try {
    const buf = await readFile(localPath);
    const b64 = buf.toString('base64');
    if (weight === 'bold') fontCacheBold = b64; else fontCacheRegular = b64;
    return b64;
  } catch { /* not on disk, fetch from CDN */ }

  // Fetch from Google Fonts CDN (TTF for librsvg compatibility)
  const urls = weight === 'bold'
    ? 'https://fonts.gstatic.com/s/inter/v18/UcC73FwrK3iLTeHuS_fjbvMwCp50PDca.ttf'
    : 'https://fonts.gstatic.com/s/inter/v18/UcCO3FwrK3iLTeHuS_fvQtMwCp50SjIw.ttf';
  const res = await fetch(urls, { signal: AbortSignal.timeout(8000) });
...
export async function renderMarketingPosterPng({ qrUrl, bandDisplayName = null, genre = null, location = null, imgUrl = null }) {
  ...
  // Embed font for cross-platform rendering (Netlify Lambda has no Segoe UI)
  let fontStyleBlock = '';
  try {
    ...
    fontStyleBlock = `<style>
      @font-face { font-family: 'PosterFont'; font-weight: 400; src: url('data:font/truetype;base64,${regularB64}') format('truetype'); }
      @font-face { font-family: 'PosterFont'; font-weight: 700; src: url('data:font/truetype;base64,${boldB64}') format('truetype'); }
      @font-face { font-family: 'PosterFont'; font-weight: 800; src: url('data:font/truetype;base64,${boldB64}') format('truetype'); }
    </style>`;
  } catch (e) {
    console.warn('marketingPoster: could not embed font, falling back', e.message);
  }

  const ff = "PosterFont, DejaVu Sans, Liberation Sans, Arial, sans-serif";
