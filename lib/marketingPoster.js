...
async function fetchImageAsDataUrl(url) {
  try {
    ...
    const resized = await sharp(buf)
      .resize(900, 400, { fit: 'cover', position: 'top' })
      .png()
      .toBuffer();
...
export async function renderMarketingPosterPng({ qrUrl, bandDisplayName = null, genre = null, location = null, imgUrl = null }) {
  ...
  // Band image — large rounded rectangle hero banner
  const imgW = 1800;
  const imgH = 680;
  const imgX = Math.round((W - imgW) / 2);
  const imgY = 140;
  const bandImgSvg = bandImgDataUrl
    ? `<defs><clipPath id="heroClip"><rect x="${imgX}" y="${imgY}" width="${imgW}" height="${imgH}" rx="36"/></clipPath></defs>
       <image href="${bandImgDataUrl}" x="${imgX}" y="${imgY}" width="${imgW}" height="${imgH}" clip-path="url(#heroClip)" preserveAspectRatio="xMidYMid slice"/>
       <rect x="${imgX}" y="${imgY}" width="${imgW}" height="${imgH}" rx="36" fill="none" stroke="url(#accentGrad)" stroke-width="4" opacity="0.5"/>`
    : `<rect x="${imgX}" y="${imgY}" width="${imgW}" height="${imgH}" rx="36" fill="#0c1929" stroke="url(#accentGrad)" stroke-width="3" opacity="0.6"/>
       <text x="${W / 2}" y="${imgY + imgH / 2 + 60}" text-anchor="middle" font-family="${ff}" font-size="240" font-weight="800" fill="#6366f1" opacity="0.5">${esc(bandName.charAt(0).toUpperCase())}</text>`;

  // Band name font size — scale for length
  const nameFontSize = bandName.length > 20 ? 130 : bandName.length > 14 ? 160 : 200;
  const nameY = imgY + imgH + 160;
