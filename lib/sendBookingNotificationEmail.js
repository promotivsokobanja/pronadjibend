...
const STATUS_LABELS = {
  PENDING: 'Na čekanju',
  BAND_ACCEPTED: 'Bend prihvatio',
  CONFIRMED: 'Potvrđeno',
  CANCELLED: 'Otkazano',
  COMPLETED: 'Završeno',
};

/** Obaveštava bend kad admin promeni status (osim CONFIRMED koji ima poseban email). */
export async function sendBookingStatusChangedToBand({ bandEmail, bandName, booking, newStatus }) {
  if (newStatus === 'CONFIRMED') return false;

  if (!bandEmail || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(bandEmail)) return false;
  ...
  const baseUrl = (process.env.NEXTAUTH_URL || 'https://pronadjibend.rs').replace(/\/$/, '');
  const dateStr = formatBookingDateSr(booking.date);
  const label = STATUS_LABELS[newStatus] || newStatus;

  await transport.sendMail({
    from,
    to: bandEmail,
    subject: `Rezervacija — status: ${label} — ${bandName}`,
    text: [
      'Zdravo,',
      '',
      `Administrator je promenio status rezervacije na: ${label}.`,
      '',
      `Bend: ${bandName}`,
      `Datum: ${dateStr}`,
      `Klijent: ${booking.clientName || '—'}`,
    ...
    html: `<p>Zdravo,</p>
<p>Administrator je promenio status rezervacije na: <strong>${escapeHtml(label)}</strong>.</p>
<ul style="line-height:1.6">
<li><strong>Bend:</strong> ${escapeHtml(bandName)}</li>
<li><strong>Datum:</strong> ${escapeHtml(dateStr)}</li>
<li><strong>Klijent:</strong> ${escapeHtml(booking.clientName || '—')}</li>
