// Direct migration runner using PrismaClient $executeRawUnsafe
...
const steps = [
  ...
  `DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'MusicianProfile_userId_fkey') THEN
      ALTER TABLE "MusicianProfile" ADD CONSTRAINT "MusicianProfile_userId_fkey"
        FOREIGN KEY ("userId") REFERENCES "User"("id") ON DELETE SET NULL ON UPDATE CASCADE;
    END IF;
  END $$`,

  // MusicianAvailability table
  ...
  `DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'MusicianAvailability_musicianId_fkey') THEN
  ...
  // LiveRequest.musicianProfileId + make bandId nullable
  `DO $$ BEGIN
  ...
  // SongSubmission.submittedByMusicianId
  `DO $$ BEGIN
  ...
  `INSERT INTO "_prisma_migrations" ("id","checksum","finished_at","migration_name","logs","rolled_back_at","started_at","applied_steps_count")
   VALUES (gen_random_uuid()::text, 'manual', NOW(), '20260405100000_musician_parity_soft_delete', NULL, NULL, NOW(), 1)
...
async function run() {
  ...
  for (let i = 0; i < steps.length; i++) {
    try {
      ...
      console.error(`  [${i + 1}/${steps.length}] ERROR: ${err.message}`);
    }
  }
